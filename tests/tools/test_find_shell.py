...
class TestMacosLoginShellSwallowRegression:
    ...
    def test_system_bash_swallows_but_zsh_does_not(self, tmp_path):
        ...
        # /bin/bash 3.2 login shell: command is swallowed (file NOT created).
        ...
        # Only assert the FIX path strongly; the bash-swallow is environment-
        # dependent (a newer bash on PATH may not swallow), so don't hard-fail
        # on it — the point is that the shell _find_shell selects works.
        assert marker_zsh.exists(), "zsh ($SHELL) path must run the command"

    def test_find_shell_selects_working_shell_on_this_box(self):
        ...
        assert os.path.exists(marker), f"_find_shell()={shell} swallowed the command"
        os.unlink(marker)
